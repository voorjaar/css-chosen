...
export function purgeCSS(html: string, css: string, minify: boolean): string {
  const styleSheet = new CSSParser(css).parse();
  return purgeStyle(html, styleSheet, minify).build(minify);
}